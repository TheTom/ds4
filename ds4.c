 ...
 *   6. Inverse rotation: signs2 → 64-point WHT → 1/sqrt(64) → signs1.  Because
 *      WHT is its own inverse but (S2·H·S1)·(S2·H·S1) != I, the forward+inverse
 *      kernel pair needs swapped sign order; applied here in one closed-form
 *      sequence so the row stays in the original basis.
 *
 ...
 * 128/256/512 sign tables used elsewhere in TQ+ are byte-identical to that fork;
 * the 64-element tables below are derived by the same Bernoulli(0.5) recipe and
 * are documented here as the canonical ds4 reference. */
...
/* Two-sided Rademacher signs for the 64-point WHT.  Generated by Pythons
...
static void dsv4_turbo3_kv_quantize_row_inplace_cpu(float *x, uint32_t head_dim, uint32_t n_rot) {
    ...
    for (uint32_t off = 0; off < n_nope; off += 64) {
        ...
        /* 6. inverse rotation: signs2 → WHT → 1/sqrt(64) → signs1.  WHT is its
         * own inverse so the same butterfly runs, but the sign order swaps.
 ...
 *   v1: original 13 u32 fields, raw KV rows stored as head_dim floats.
 *   v2: upstream ABI bump that invalidates stale v1 payloads after recent
 *       runtime layout changes.  Same 13-field header; kv_dtype is still
 *       implicit fp8.
 *   v3: adds one u32 kv_dtype field (DS4_KV_FP8 or DS4_KV_TURBO3).  When
 *       the saved dtype is DS4_KV_TURBO3, raw KV rows are stored at the
 *       packed byte stride ds4_kv_row_bytes(head_dim, n_rot, DS4_KV_TURBO3)
 *       instead of head_dim*4.  Compressor and indexer state stay as
 *       floats either way.
 *
