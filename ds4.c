...
/* =========================================================================
 * TurboQuant+ turbo3 KV quality simulation.
 * =========================================================================
 *
 * Sibling of dsv4_fp8_kv_quantize_row_inplace_cpu.  Same group-of-64 structure;
 * same in-place "consume float row, write float row with quant error baked in"
 * contract; same RoPE-tail (last n_rot elements) left untouched.
 *
 * Per group the round trip is:
 *   1. Randomized Hadamard rotation: signs1 (Rademacher mask) -> 64-point WHT
 *      -> 1/sqrt(64) normalize -> signs2 (Rademacher mask).  Gaussianizes the
 ...
 *      activation distribution.  Both sign tables come from Python's
 *      random.Random(seed) with Bernoulli(0.5) - deterministic and documented
 *      below.
 ...
 *      clamp the scale to the FP8 E4M3 representable range so a Metal port
 *      that stores the scale as packed FP8 stays bit-equivalent.
 *   5. Lookup dequantized centroid * matched-norm scale.
 *   6. Inverse rotation: signs2 -> 64-point WHT -> 1/sqrt(64) -> signs1.  WHT
 *      is its own inverse but (S2.H.S1).(S2.H.S1) != I, so the forward+inverse
 *      pair needs swapped sign order; applied here in one closed-form sequence
 *      so the row stays in the original basis.
 *
 * The diagnostic switch DS4_TURBO_NO_SIGNS=1 in the environment drops the
 * Rademacher masks (all-positive signs).  Plain WHT is strictly weaker than
 * the canonical Randomized Hadamard form and is provided only for A/B testing
 * the sign contribution to PPL - not a release path.
 *
 * Prior art chain: Google TurboQuant (arXiv:2504.19874, ICLR 2026) ->
 * TheTom/turboquant_plus umbrella -> TheTom/llama-cpp-turboquant engine
 * reference.  The 128/256/512 sign tables used elsewhere in TQ+ are
 * byte-identical to that fork; the 64-element tables below are derived by the
 * same Bernoulli(0.5) recipe and are documented here as the canonical ds4
 * reference. */

/* Lloyd-Max 8-level codebook for N(0,1).  MSE = 0.03454 vs raw N(0,1) sample.
 * Centroids and decision boundaries match TURBO3_CODEBOOK / TURBO3_BOUNDS in
 * Atlas's reshape_and_cache_turbo.cu and the corresponding LUT in the
 * paged_decode_attn_turbo3 dequant path. */
static const float DS4_TURBO3_CODEBOOK[8] = {
    -2.1520f, -1.3440f, -0.7560f, -0.2451f, 0.2451f, 0.7560f, 1.3440f, 2.1520f
};
static const float DS4_TURBO3_BOUNDS[7] = {
    -1.748f, -1.050f, -0.501f, 0.0f, 0.501f, 1.050f, 1.748f
};
#define DS4_TURBO3_MAX 2.1520f

/* Lloyd-Max 16-level codebook for N(0,1).  MSE = 0.009497 - ~3.6x lower
 * quantization noise than the 8-level turbo3 codebook above, at the cost
 * of 32 B / 64-elem group (vs turbo3's 24 B).  Centroids + boundaries
 * match TURBO4_CODEBOOK / TURBO4_BOUNDS in Atlas's reshape_and_cache_turbo.cu. */
static const float DS4_TURBO4_CODEBOOK[16] = {
...
static const float DS4_TURBO4_BOUNDS[15] = {
    -2.4008f, -1.8435f, -1.4371f, -1.0993f, -0.7996f, -0.5224f, -0.2582f, 0.0f,
     0.2582f,  0.5224f,  0.7996f,  1.0993f,  1.4371f,  1.8435f,  2.4008f
};
#define DS4_TURBO4_MAX 2.7326f

/* FP8 E4M3 max representable.  Matched-norm scale is clamped here so a future
 * Metal storage path can pack the scale into one FP8 byte per 64-element group
 * without an extra renormalization pass. */
#define DS4_FP8_E4M3_MAX 448.0f

/* Two-sided Rademacher signs for the 64-point WHT.  Generated by Python's
 * random.Random(seed) with Bernoulli(0.5) mapped to {-1,+1}.  Same canonical
 * approach as the 128/256/512 tables vendored into TheTom/llama-cpp-turboquant
 * - only the length and seed differ, picked here to match ds4's natural
 * per-group cadence of 64.
 *
...
static const float DS4_TURBO_SIGNS2_64[64] = {
    +1.0f, +1.0f, -1.0f, -1.0f, -1.0f, +1.0f, -1.0f, -1.0f, -1.0f, +1.0f, +1.0f, +1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
    -1.0f, +1.0f, -1.0f, +1.0f, +1.0f, -1.0f, -1.0f, +1.0f, -1.0f, -1.0f, +1.0f, -1.0f, +1.0f, +1.0f, +1.0f, -1.0f,
    -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, +1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, +1.0f, -1.0f, -1.0f, -1.0f,
    +1.0f, +1.0f, +1.0f, +1.0f, -1.0f, +1.0f, -1.0f, -1.0f, -1.0f, +1.0f, -1.0f, +1.0f, +1.0f, -1.0f, -1.0f, +1.0f,
...
/* Walsh-Hadamard butterfly on a 64-element scratch.  Self-inverse up to the
 * 1/sqrt(64) normalization (applied once by the caller in each direction). */
static void dsv4_turbo3_wht64_inplace_cpu(float *v) {
    for (uint32_t stride = 1; stride < 64; stride <<= 1) {
        for (uint32_t base = 0; base < 64; base += 2u * stride) {
            for (uint32_t i = 0; i < stride; i++) {
                const float a = v[base + i];
                const float b = v[base + stride + i];
                v[base + i] = a + b;
                v[base + stride + i] = a - b;
            }
        }
...
/* Nearest-centroid lookup against DS4_TURBO3_BOUNDS.  Branchless-ish: one
 * 3-way descent matching the binary search in Atlas's turbo3_quantize. */
static int dsv4_turbo3_quantize_index_cpu(float x) {
    int idx;
    if (x >= DS4_TURBO3_BOUNDS[3]) {  /* >= 0 */
        idx = 4;
        if (x >= DS4_TURBO3_BOUNDS[5]) { idx = 6; if (x >= DS4_TURBO3_BOUNDS[6]) idx = 7; }
        else if (x >= DS4_TURBO3_BOUNDS[4]) idx = 5;
    } else {
        idx = 0;
        if (x >= DS4_TURBO3_BOUNDS[1]) { idx = 2; if (x >= DS4_TURBO3_BOUNDS[2]) idx = 3; }
        else if (x >= DS4_TURBO3_BOUNDS[0]) idx = 1;
    }
    return idx;
}

/* Cached env query for the diagnostic no-signs switch.  Reads once per process.
 * Returns 1 when the env var is set to a non-empty, non-"0" value. */
static int dsv4_turbo_signs_enabled_cpu(void) {
    static int cached = -1;
    if (cached < 0) {
...
/* Linear scan over 15 boundaries -> 4-bit idx [0..15].  Same shape as turbo3
 * quant - branch tree expanded but with twice the levels. */
static int dsv4_turbo4_quantize_index_cpu(float x) {
    int code = 0;
    for (int j = 0; j < 15; j++) {
        if (x >= DS4_TURBO4_BOUNDS[j]) code = j + 1;
    }
    return code;
}

/* turbo4 sibling of dsv4_turbo3_kv_quantize_row_inplace_cpu - same WHT +
 * matched-norm L2 + iWHT sequence, only the per-element quant table changes. */
static void dsv4_turbo4_kv_quantize_row_inplace_cpu(float *x, uint32_t head_dim, uint32_t n_rot) {
    const uint32_t n_nope = head_dim - n_rot;
    ...
    for (uint32_t off = 0; off < n_nope; off += 64) {
        if (signs_on) {
            for (uint32_t i = 0; i < 64; i++) buf[i] = x[off + i] * DS4_TURBO_SIGNS1_64[i];
        } else {
            for (uint32_t i = 0; i < 64; i++) buf[i] = x[off + i];
        }
        dsv4_turbo3_wht64_inplace_cpu(buf);
        const float inv_sqrt_n = 1.0f / sqrtf(64.0f);
        for (uint32_t i = 0; i < 64; i++) buf[i] *= inv_sqrt_n;
        if (signs_on) {
            for (uint32_t i = 0; i < 64; i++) buf[i] *= DS4_TURBO_SIGNS2_64[i];
        }
        ...
        const float k_inv = (amax > 1e-12f) ? (DS4_TURBO4_MAX / amax) : 1.0f;

        int idx[64];
        float recon_sq = 0.0f;
        for (uint32_t i = 0; i < 64; i++) {
            idx[i] = dsv4_turbo4_quantize_index_cpu(buf[i] * k_inv);
            const float c = DS4_TURBO4_CODEBOOK[idx[i]];
            recon_sq += c * c;
        }
        const float recon_norm = sqrtf(recon_sq);
        float scale = (recon_norm > 1e-10f) ? (sqrtf(norm_sq) / recon_norm) : (amax / DS4_TURBO4_MAX);
        if (scale > DS4_FP8_E4M3_MAX) scale = DS4_FP8_E4M3_MAX;

        for (uint32_t i = 0; i < 64; i++) {
            buf[i] = DS4_TURBO4_CODEBOOK[idx[i]] * scale;
        }
        ...
        dsv4_turbo3_wht64_inplace_cpu(buf);
        const float inv_sqrt_n2 = 1.0f / sqrtf(64.0f);
        for (uint32_t i = 0; i < 64; i++) buf[i] *= inv_sqrt_n2;
        if (signs_on) {
            for (uint32_t i = 0; i < 64; i++) buf[i] *= DS4_TURBO_SIGNS1_64[i];
        }

        for (uint32_t i = 0; i < 64; i++) x[off + i] = buf[i];
    }
}

/* In-place turbo3 quality round trip on one MLA latent KV row.  RoPE tail (last
 * n_rot elements) is left untouched, matching dsv4_fp8_kv_quantize_row_inplace_cpu.
 *
 * Group boundaries align with the existing FP8 path (every 64 floats), so the
 * surrounding cache code is dtype-agnostic.  Storage layout unchanged. */
static void dsv4_turbo3_kv_quantize_row_inplace_cpu(float *x, uint32_t head_dim, uint32_t n_rot) {
    const uint32_t n_nope = head_dim - n_rot;
    const int signs_on = dsv4_turbo_signs_enabled_cpu();
    float buf[64];

    for (uint32_t off = 0; off < n_nope; off += 64) {
        /* 1. forward Randomized Hadamard rotation. */
        if (signs_on) {
            for (uint32_t i = 0; i < 64; i++) buf[i] = x[off + i] * DS4_TURBO_SIGNS1_64[i];
        } else {
            for (uint32_t i = 0; i < 64; i++) buf[i] = x[off + i];
        ...
        /* 2. per-group amax -> scale into the codebook range. */
        float amax = 0.0f, norm_sq = 0.0f;
        for (uint32_t i = 0; i < 64; i++) {
            const float v = buf[i];
            const float av = fabsf(v);
        ...
        /* 3+4. Lloyd-Max quantize and accumulate centroid recon L2 norm.
         * Matched-norm scale = ||original|| / ||centroid_recon||.  Falls back
         * to the amax scale on degenerate (all-zero or all-clipped) groups. */
        int idx[64];
        float recon_sq = 0.0f;
        for (uint32_t i = 0; i < 64; i++) {
            idx[i] = dsv4_turbo3_quantize_index_cpu(buf[i] * k_inv);
            const float c = DS4_TURBO3_CODEBOOK[idx[i]];
            recon_sq += c * c;
        }
        const float recon_norm = sqrtf(recon_sq);
        float scale = (recon_norm > 1e-10f) ? (sqrtf(norm_sq) / recon_norm) : (amax / DS4_TURBO3_MAX);
        if (scale > DS4_FP8_E4M3_MAX) scale = DS4_FP8_E4M3_MAX;

        /* 5. dequantize centroid * matched-norm scale, writing back into buf. */
        for (uint32_t i = 0; i < 64; i++) {
            buf[i] = DS4_TURBO3_CODEBOOK[idx[i]] * scale;
        }

        /* 6. inverse rotation: signs2 -> WHT -> 1/sqrt(64) -> signs1.  WHT is
         * its own inverse so the same butterfly runs, but the sign order swaps.
         * The first signs2 mask cancels the post-WHT signs2 from the forward
         * pass; the trailing signs1 cancels the pre-WHT signs1. */
        if (signs_on) {
            for (uint32_t i = 0; i < 64; i++) buf[i] *= DS4_TURBO_SIGNS2_64[i];
        }
        dsv4_turbo3_wht64_inplace_cpu(buf);
...
/* =========================================================================
 * Packed-turbo3 byte-level helpers.
 * =========================================================================
 *
 ...
 * which matches what the in-place float-sim quantizer above wrote for the
 ...
 * WHT+signs1+signs2 pipeline used by the float-sim quantizer above - see
...
static void dsv4_turbo3_pack_group64_cpu(
    ...
    /* Quantize + matched-norm scale (same algorithm as the float-sim path). */
    int idx[64];
    float recon_sq = 0.0f;
    for (int i = 0; i < 64; i++) {
 ...
 *
 * Read concurrency: written exactly once before any session is created.  The
 * helper readers are all called from the single session thread, so no atomic
 ...
 * gates (allocate-as-bytes, dequant-to-scratch round-trip, etc).  When the
 * specific bit width matters (kernel selection, ppl reference), branch on
 * the dtype directly. */
static inline int ds4_kv_dtype_is_packed_byte(ds4_kv_dtype d) {
    return d == DS4_KV_TURBO3 || d == DS4_KV_TURBO4;
}
...
static void ds4_kv_quantize_row_inplace_cpu(float *x, uint32_t head_dim, uint32_t n_rot) {
    if (g_ds4_kv_dtype == DS4_KV_TURBO3) {
        dsv4_turbo3_kv_quantize_row_inplace_cpu(x, head_dim, n_rot);
    } else if (g_ds4_kv_dtype == DS4_KV_TURBO4) {
        dsv4_turbo4_kv_quantize_row_inplace_cpu(x, head_dim, n_rot);
    } else {
        dsv4_fp8_kv_quantize_row_inplace_cpu(x, head_dim, n_rot);
    }
}
 ...
 *
 * On the turbo3 path: `_quantize_tensor_dispatch` still runs the float-sim
 ...
 * Sites that store into the per-layer raw_cache go through the
...
static int ds4_gpu_kv_quantize_tensor_dispatch(
    ...
    if (g_ds4_kv_dtype == DS4_KV_TURBO4) {
        return ds4_gpu_dsv4_turbo4_kv_quantize_tensor(x, n_tok, head_dim, n_rot);
    }
...
static int ds4_gpu_kv_store_raw_tensor_dispatch(
    ...
    if (g_ds4_kv_dtype == DS4_KV_TURBO4) {
        const uint64_t row_bytes = ds4_kv_row_bytes(head_dim, n_rot, DS4_KV_TURBO4);
        return ds4_gpu_dsv4_turbo4_kv_pack_batch_tensor(
                kv, raw_cache, raw_cap, row, 1, head_dim, n_rot, row_bytes);
    }
...
static ds4_gpu_tensor *ds4_gpu_kv_attention_view_dispatch(
    ...
    if (g_ds4_kv_dtype == DS4_KV_TURBO4) {
        if (!scratch || !raw_cache) return NULL;
        const uint64_t row_bytes = ds4_kv_row_bytes(head_dim, n_rot, DS4_KV_TURBO4);
        if (ds4_gpu_dsv4_turbo4_kv_dequant_to_scratch_tensor(
                raw_cache, scratch, raw_cap, head_dim, n_rot, row_bytes) == 0) {
            return NULL;
        }
        return scratch;
...
const char *ds4_kv_dtype_name(ds4_kv_dtype dtype) {
    switch (dtype) {
        case DS4_KV_TURBO3: return "turbo3";
        case DS4_KV_TURBO4: return "turbo4";
        case DS4_KV_FP8:    return "fp8";
        default:            return "fp8";
    }
}
...
int ds4_kv_dtype_from_name(const char *name, ds4_kv_dtype *out) {
    ...
    if (!strcmp(name, "fp8"))    { *out = DS4_KV_FP8;    return 1; }
    if (!strcmp(name, "turbo3")) { *out = DS4_KV_TURBO3; return 1; }
    if (!strcmp(name, "turbo4")) { *out = DS4_KV_TURBO4; return 1; }
    return 0;
}
...
static bool compressor_decode_one(
    ...
    if (head_dim == DS4_N_HEAD_DIM) {
        ds4_kv_quantize_row_inplace_cpu(out_comp, head_dim, DS4_N_ROT);
...
static void layer_attention_raw_swa_one(
    ...
    ds4_kv_quantize_row_inplace_cpu(kv, DS4_N_HEAD_DIM, DS4_N_ROT);
...
static void layer_attention_raw_swa_batch(
    ...
    for (uint32_t t = 0; t < n_tok; t++) {
        ...
        ds4_kv_quantize_row_inplace_cpu(kv_t, DS4_N_HEAD_DIM, DS4_N_ROT);
...
static void layer_forward_raw_swa_one(
    ...
    ds4_kv_quantize_row_inplace_cpu(scratch->kv, DS4_N_HEAD_DIM, DS4_N_ROT);
...
typedef struct {
    ...
    /* Turbo3 dequant scratch.  When the active dtype is DS4_KV_TURBO3 the
     * layer_raw_cache buffers are sized as packed bytes (~431 B/row vs
     * 2048 B/row for fp8), so attention kernels that lack an inline-dequant
     * sibling can't read them directly.  Before each such attention dispatch
     * the dequant kernel unpacks raw_cap rows from layer_raw_cache[il] into
     * this scratch tensor.  For fp8 this stays NULL and the attention kernel
     * reads layer_raw_cache directly.
     *
     * Trade-off: real packed cache memory savings on layer_raw_cache (a
     * ~9.6 MB shrink on the SWA ring at raw_cap=128, DS4_N_LAYER=43); the
     * dequant-to-scratch pass adds ~5 us per attention call at decode T=1
     * (negligible on GX10).  The inline-dequant attention kernels below skip
     * this hop entirely and read packed rows directly to capture the V-load
     * bandwidth win. */
...
static bool metal_graph_alloc_raw_cap(
    ...
    if (ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)) {
...
static bool metal_graph_decode_kv_store(
    ...
    if (metal_graph_use_reference_kv_decode()) {
        return ds4_gpu_kv_quantize_tensor_dispatch(kv, 1, DS4_N_HEAD_DIM, DS4_N_ROT) != 0 &&
               ds4_gpu_store_raw_kv_tensor(raw_cache, kv, raw_cap, raw_row, DS4_N_HEAD_DIM) != 0;
    }

    return ds4_gpu_kv_store_raw_tensor_dispatch(kv,
                                                  raw_cache,
                                                  raw_cap,
                                                  raw_row,
                                                  DS4_N_HEAD_DIM,
...
/* Phase 7.5: variant that EXPOSES the packed pool to an inline-dequant
 * attention launcher.  Writes the per-row stride (in bytes) into
 * *out_row_bytes:
 *   - 0  -> legacy float layout (caller may use returned tensor as
 *           `const float *` of head_dim floats per row).
 *   - >0 -> turbo3-packed bytes per row; caller MUST pass this value
 *           to the launcher so the kernel knows to inline-dequant.
 *
 * **Disabled by default**.  Empirical finding: at long ctx (n_comp >
 * ~512) inline-dequant comp loses to dequant-to-scratch because the
 * dequant work is redundant across head_groups (each attention CTA
 * dequants the same comp rows independently, vs the upfront kernel
 * which dequants once across all CTAs).  Phase 7.4 dequant-to-scratch
 * remains the right architecture for comp_kv.  Set DS4_GPU_INLINE_COMP_KV=1
 * to enable for A/B testing on short contexts.
 *
 * The wiring (kernel branches, launcher params, dispatcher plumbing)
 * stays in place because (a) the K-dot/V-acc skeleton is correct, just
 * compute-redundant, and (b) future work (per-tile sparse-V skip,
 * indexed comp selection) can re-enable it for cases where it pays. */
static int metal_graph_comp_kv_inline_enabled(void) {
    static int cached = -1;
    if (cached < 0) {
        const char *env = getenv("DS4_GPU_INLINE_COMP_KV");
        cached = (env && env[0] && env[0] != '0') ? 1 : 0;
    }
    return cached;
}
static ds4_gpu_tensor *metal_graph_comp_kv_packed_for_attn(
        ds4_gpu_graph *g, uint32_t il, uint32_t n_comp,
    ...
    if (DS4_GPU_ATTN_COMP_CACHE_F16 || !metal_graph_comp_kv_inline_enabled()) {
        return metal_graph_comp_kv_for_attn(g, il, n_comp);
    }
    if (n_comp == 0 || g->layer_attn_comp_cache_packed[il] == NULL) {
        return metal_graph_comp_kv_for_attn(g, il, n_comp);
    }
    if (out_row_bytes) {
        *out_row_bytes = ds4_comp_row_bytes(DS4_N_HEAD_DIM, DS4_KV_TURBO3);
    }
    return g->layer_attn_comp_cache_packed[il];
}
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
    ...
    /* Turbo3 read path: the layer raw_cache is byte-packed (431 B/row vs
     * 2048 B/row for fp8) so existing float-reading attention kernels can't
     * dereference it as `float *raw_kv`.
     *
     * Kernels with inline-dequant siblings (the decode_heads simple path via
     * attention_decode_mixed_turbo3_kernel) read packed bytes directly - no
     * view_dispatch hop needed.  Kernels without an inline-dequant sibling
     * still go through view_dispatch which dequants into the per-graph
     * scratch float tensor.
     *
     * We defer the view_dispatch call to the attention branch below where we
     * know which kernel runs.  raw_cache (packed bytes in turbo3, float in
     * fp8) is passed into the branch unmodified. */
    ...
    /* Phase 7.5: when comp_cache points at the turbo3-packed pool, this
     * holds the per-row byte stride; the inline-dequant attention
     * launchers branch on it (0 = legacy float). */
    uint64_t comp_row_bytes = 0;
    ...
    if (ok && compressed) {
        ...
        /* Phase 7.5: prefer the packed pool (no dequant-to-scratch pre-pass).
         * Falls back to the dequant scratch when inline path is disabled
         * (F16 staging, escape hatch, or n_comp==0). */
        comp_cache = metal_graph_comp_kv_packed_for_attn(g, il, n_comp, &comp_row_bytes);
    }
    ...
    if (ok) {
        ...
        if (n_comp != 0 && comp_selected != NULL && n_selected != 0) {
            /* Indexed mixed path.  The inline-dequant turbo3 sibling covers
             * the n_tokens=1 fallback kernel (decode-token, the hot path).
             * heads8_online / rb4 paths still need float - fall back to
             * view_dispatch when the turbo3 launcher returns 0. */
            int turbo3_rc = 0;
            if (g_ds4_kv_dtype == DS4_KV_TURBO3) {
            ...
            if (turbo3_rc == 0) {
                ds4_gpu_tensor *raw_cache_attn = ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)
        ...
        } else if (ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)) {
            /* Inline-dequant turbo3/turbo4 sibling of decode_heads: pass
             * packed bytes directly, no view_dispatch dequant.  Launcher
             * returns 0 (falls back to dequant-to-scratch + fp8 attention)
...
static void metal_graph_trace_layer_stages(
    ...
    ds4_kv_quantize_row_inplace_cpu(cpu_kv, DS4_N_HEAD_DIM, DS4_N_ROT);
...
#define DS4_METAL_PROFILE_Q_STAGE(name) do { \
    ...
    if (ok) ok = ds4_gpu_kv_quantize_tensor_dispatch(g->batch_kv,
                                                       n_tokens,
                                                       DS4_N_HEAD_DIM,
                                                       DS4_N_ROT) != 0;
    if (ok) {
    ...
    if (ok && raw_batch_attention) {
        ...
        /* Prefill-chunk raw batch.  Try turbo3 heads8_online via the turbo3
         * launcher first; fall back to float dequant-to-scratch + existing
         * kernel on rc==0. */
        int turbo3_rc = 0;
        if (g_ds4_kv_dtype == DS4_KV_TURBO3) {
            const uint64_t row_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, DS4_KV_TURBO3);
        ...
        if (turbo3_rc == 0) {
            ds4_gpu_tensor *raw_cache_attn = ok ? (ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)
                    ? ds4_gpu_kv_attention_view_dispatch(
            ...
            if (aligned_chunk) {
                ...
                for (uint32_t t = 0; ok && t < n_tokens; t++) {
                    ...
                    if (ok && emit) {
                        ds4_gpu_tensor *comp_row_view = metal_graph_attn_comp_row_view(g, il, comp_row);
                        ok = comp_row_view &&
                             ds4_gpu_kv_quantize_tensor_dispatch(comp_row_view,
                                                                   1,
                                                                   DS4_N_HEAD_DIM,
                                                                   DS4_N_ROT) != 0;
                        if (ok) {
        ...
        if (ok && !zero_prefix && n_tokens <= g->raw_cap) {
            ...
            /* Try the turbo3 inline-dequant launcher first; on rc==0 fall
             * back to view_dispatch + float kernel. */
            int turbo3_rc_a = 0;
            if (g_ds4_kv_dtype == DS4_KV_TURBO3 && use_indexed_comp) {
                const uint64_t row_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, DS4_KV_TURBO3);
                uint64_t comp_row_bytes_a = 0;
                ds4_gpu_tensor *comp_cache_a = metal_graph_comp_kv_packed_for_attn(
                        g, il, n_comp, &comp_row_bytes_a);
            ...
            if (turbo3_rc_a == 0 && ok) {
                raw_cache_attn_a = ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)
                        ? ds4_gpu_kv_attention_view_dispatch(
                                g->layer_raw_cache[il], g->raw_cache_dequant_scratch,
                                g->raw_cap, DS4_N_HEAD_DIM, DS4_N_ROT)
                        : g->layer_raw_cache[il];
        ...
        if (ok && zero_prefix && topk_prefill_needed && n_comp != 0) {
            ...
            if (g_ds4_kv_dtype == DS4_KV_TURBO3) {
                const uint64_t row_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, DS4_KV_TURBO3);
                uint64_t comp_row_bytes_b = 0;
                ds4_gpu_tensor *comp_cache_b = metal_graph_comp_kv_packed_for_attn(
                        g, il, n_comp, &comp_row_bytes_b);
            ...
            if (turbo3_rc_b == 0) {
                ds4_gpu_tensor *raw_cache_attn_b = ok ? (ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)
                        ? ds4_gpu_kv_attention_view_dispatch(
                                g->layer_raw_cache[il], g->raw_cache_dequant_scratch,
                                g->raw_cap, DS4_N_HEAD_DIM, DS4_N_ROT)
                        : g->layer_raw_cache[il]) : NULL;
        ...
        if (raw_prefix_tokens < n_tokens) {
            for (uint32_t t = raw_prefix_tokens; ok && t < n_tokens; t++) {
                ...
                if (g_ds4_kv_dtype == DS4_KV_TURBO3 && ok) {
                    const uint64_t row_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, DS4_KV_TURBO3);
                    uint64_t comp_row_bytes_c = 0;
                    ds4_gpu_tensor *comp_cache_c = metal_graph_comp_kv_packed_for_attn(
                            g, il, cur_comp, &comp_row_bytes_c);
                ...
                if (turbo3_rc_c == 0) {
                    ds4_gpu_tensor *raw_cache_attn_c = ok ? (ds4_kv_dtype_is_packed_byte(g_ds4_kv_dtype)
                            ? ds4_gpu_kv_attention_view_dispatch(
                                    g->layer_raw_cache[il], g->raw_cache_dequant_scratch,
                                    g->raw_cap, DS4_N_HEAD_DIM, DS4_N_ROT)
                            : g->layer_raw_cache[il]) : NULL;
...
struct ds4_engine {
    ...
    bool metal_ready;
    bool mtp_ready;
    /* KV cache dtype: DS4_KV_FP8 (default, historical path) or DS4_KV_TURBO3
     * (TurboQuant+ quality simulation on the compressed-KV non-RoPE part).
...
ds4_kv_footprint ds4_kv_footprint_estimate(ds4_backend backend, int ctx_size, ds4_kv_dtype dtype) {
     ...
     * float / F16 because the compressor pool integrates softmax-weighted
     * accumulations that require an original-basis read. */
 ...
 *   v1: 13 u32 fields, raw KV rows stored as head_dim floats.
 *   v2: upstream ABI bump that invalidates stale v1 payloads after
 *       recent runtime layout changes.  Same 13-field header.
 *   v3: adds a kv_dtype field (DS4_KV_FP8 or DS4_KV_TURBO3).  Turbo3
 *       saves raw KV rows at the packed byte stride from
 *       ds4_kv_row_bytes() instead of head_dim*4.  Compressor and
 *       indexer state stay as floats either way.
 *
 * Backward compat: a v3 reader accepts v1 and v2 files by falling back
 * to the legacy 13-field header and assuming kv_dtype = DS4_KV_FP8.
 * Older binaries see a v3 file as "unsupported session payload version"
 * and refuse to load.  A v3 reader also compares the saved kv_dtype
 * against the active engine dtype and rejects a mismatch outright. */
#define DS4_SESSION_PAYLOAD_VERSION UINT32_C(3)
#define DS4_SESSION_PAYLOAD_VERSION_V2 UINT32_C(2)
#define DS4_SESSION_PAYLOAD_VERSION_V1 UINT32_C(1)
#define DS4_SESSION_PAYLOAD_U32_FIELDS 14u
#define DS4_SESSION_PAYLOAD_U32_FIELDS_LEGACY 13u
...
static uint64_t session_payload_live_tensor_bytes(const ds4_gpu_graph *g, uint32_t checkpoint_len) {
    ...
    /* Disk v2: raw rows are stored at the per-dtype packed byte stride when
     * dtype=turbo3.  fp8 path stays at head_dim*4 (v1-equivalent). */
    const uint64_t raw_row_disk_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, g_ds4_kv_dtype);
    /* Phase 7.4: comp_cache rows are packed turbo3 bytes when comp_dtype=TURBO3
     * (no float pool, no RoPE tail).  Mirror the save side's per-row size. */
    const uint64_t comp_row_disk_bytes = (g_ds4_comp_dtype == DS4_KV_TURBO3)
        ? ds4_comp_row_bytes(DS4_N_HEAD_DIM, DS4_KV_TURBO3)
        : (uint64_t)DS4_N_HEAD_DIM * sizeof(float);
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        const uint32_t ratio = ds4_layer_compress_ratio(il);
        if (ratio == 0) continue;
        bytes += (uint64_t)g->layer_n_comp[il] * comp_row_disk_bytes;
...
int ds4_session_save_payload(ds4_session *s, FILE *fp, char *err, size_t errlen) {
    ...
    for (uint32_t il = 0; rc == 0 && il < DS4_N_LAYER; il++) {
        ...
        /* Disk v2 write: stream raw bytes from the cache at the per-dtype
         * row stride.  fp8 writes head_dim*4 bytes/row (unchanged from v1);
         * turbo3 writes the packed turbo3 layout directly (no dequant pass
         * on save). */
        const uint64_t raw_row_disk_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, g_ds4_kv_dtype);
        ...
        if (g_ds4_comp_dtype == DS4_KV_TURBO3) {
            /* Phase 7.4: float comp pool was never allocated; save the
             * packed pool directly.  Smaller payload (~4.75x), and load
             * side reads it straight back into the same packed pool. */
            const uint64_t comp_row_bytes = ds4_comp_row_bytes(DS4_N_HEAD_DIM, DS4_KV_TURBO3);
            rc = payload_write_tensor_span(fp,
                                           g->layer_attn_comp_cache_packed[il],
                                           0,
                                           (uint64_t)g->layer_n_comp[il] * comp_row_bytes,
                                           buf,
                                           DS4_SESSION_IO_CHUNK,
                                           err,
                                           errlen);
...
int ds4_session_load_payload(ds4_session *s, FILE *fp, uint64_t payload_bytes, char *err, size_t errlen) {
    ...
    /* Read magic + version first.  v3 stores the kv_dtype explicitly in
     * h[13]; v1 and v2 use the legacy 13-field header where kv_dtype is
     * implicit fp8.  Older binaries refuse to load a v3 file. */
    ...
    } else if (h[1] == DS4_SESSION_PAYLOAD_VERSION_V2 ||
               h[1] == DS4_SESSION_PAYLOAD_VERSION_V1) {
        header_fields = DS4_SESSION_PAYLOAD_U32_FIELDS_LEGACY;
    } else {
        payload_set_err(err, errlen, "unsupported session payload version");
        return 1;
    }
    ...
    /* h[13] is the saved kv_dtype; v1/v2 files keep it at the zero-init
     * value DS4_KV_FP8 which is correct (legacy formats only stored
     * fp8 floats). */
    ...
    uint8_t *buf = xmalloc(DS4_SESSION_IO_CHUNK);
    int rc = 0;
    /* Disk v2 read: rows are stored at the per-dtype packed byte stride
     * (matches the in-memory cache layout).  v1 files always have fp8 dtype,
     * head_dim*4 bytes/row.  v2 turbo3 files have packed bytes. */
    const uint64_t raw_row_disk_bytes = ds4_kv_row_bytes(DS4_N_HEAD_DIM, DS4_N_ROT, g_ds4_kv_dtype);
    for (uint32_t il = 0; rc == 0 && il < DS4_N_LAYER; il++) {
        ...
        if (rc != 0 || ratio == 0) continue;
        if (g_ds4_comp_dtype == DS4_KV_TURBO3) {
            /* Phase 7.4: packed-bytes load mirroring the save side. */
            const uint64_t comp_row_bytes = ds4_comp_row_bytes(DS4_N_HEAD_DIM, DS4_KV_TURBO3);
            rc = payload_read_tensor_span(fp,
                                          g->layer_attn_comp_cache_packed[il],
                                          0,
                                          (uint64_t)n_comp[il] * comp_row_bytes,
                                          buf,
                                          DS4_SESSION_IO_CHUNK,
                                          &remaining,
                                          err,
...
int ds4_engine_head_test(ds4_engine *e, const ds4_tokens *prompt) {
    ...
    ds4_kv_quantize_row_inplace_cpu(kv0, DS4_N_HEAD_DIM, DS4_N_ROT);
